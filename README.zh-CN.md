...
浏览器 `hook.wasm` 和服务端 `--hook` 都会在解密后修复可确认的 ALAC 包尾错误（如歌曲 `1691044818`）：根据 init 中的轨道与 sample description，定位 PCM 完整的未压缩单声道／立体声包，将缺失或损坏的 3-bit `TYPE_END` 恢复为 `111`。修复不改变 PCM、sample 长度或 Range 偏移，因此播放和下载均可受益。压缩包、PCM 截断及没有足够尾部空间的包不做原地修复；转 FLAC 时仍保留可追加结束标记的兜底。
...
浏览器端解密核心 `src/ui/hook.wasm` 与 ALAC 转 FLAC 核心 `src/ui/flac.wasm` 都随仓库提交，并在构建服务端时内嵌。`flac.wasm` 仅在需要 ALAC 转码时加载。修改相关 crate 后需要重新生成并提交产物：
